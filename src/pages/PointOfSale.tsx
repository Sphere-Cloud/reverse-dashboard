...
const PointOfSale = () => {
  ...
  const addToCart = (cartItem: CartItem) => {
    ...
    if (productIndex !== -1) {
      ...
      updatedCart[productIndex].quantity += cartItem.quantity;
      setCart(updatedCart);
    } else {
      setCart([...cart, { ...cartItem, quantity: cartItem.quantity }]);
    }
  };
  ...
  const calcTotal = () => {
    return cart.reduce(
      (sum, item) => sum + item.inventory.price_sale * item.quantity,
      0
    );
  };
  ...
  const saveData = async () => {
    try {
      ...
      const parsedCompanyId = companyId
        ? parseFloat(JSON.parse(companyId))
        : null;
    ...
    <Container fluid style={{ height: '100%' }}>
      ...
      {!isPaying ? (
        ...
        <Row style={{ height: '90%', padding: '0' }}>
                ...
                style={{
                  ...
                  <Col>
                    <Form.Group>
                      ...
                      >Método de Pago {index + 1}</Form.Label>
                      <Form.Control
                        as="select"
          ...
          <Col
            md={6}
            style={{ background: '#FFF6EF', border: '2px solid #D35711' }}
          >
            <div
              style={{
                borderBottom: '2px solid #D35711',
                height: '30%',
                display: 'flex',
